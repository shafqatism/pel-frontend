...
export const hrApi = {
  employees: {
    ...
    importBulk: (dtos: CreateEmployeeDto[]) =>
      api.post<{ count: number }>("/hr/employees/bulk", dtos).then(r => r.data),
  ...
  attendance: {
    ...
    getHeadcount: (site: string, date: string) =>
      api.get<{ count: number }>("/hr/attendance/headcount", { params: { site, date } }).then(r => r.data),
  },

  reports: {
    export: (type: string, format: string) =>
      api.get(`/hr/export/${type}`, { params: { format }, responseType: 'blob' }),
  },
}
