...
export const companiesApi = {
  ...
  importBulk: (dtos: Partial<CreateCompanyDto>[]) =>
    api.post<{ count: number }>("/companies/bulk", dtos).then(r => r.data),

  reports: {
    export: (format: string) =>
      api.get("/companies-management/export", { params: { format }, responseType: 'blob' }),
  },
}
