...
export function LoginPage() {
  ...
  if (showSplash) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-white dark:bg-black">
         <style>
            {`
               @keyframes zoomOutInSplash {
                  0% { transform: scale(1.5); opacity: 0; filter: blur(10px); }
                  50% { transform: scale(0.85); opacity: 1; filter: blur(0px); }
                  100% { transform: scale(1); opacity: 1; filter: blur(0px); }
               }
               .animate-splash {
                  animation: zoomOutInSplash 1.2s cubic-bezier(0.25, 1, 0.5, 1) forwards;
               }
            `}
         </style>
         <div className="animate-splash flex flex-col items-center">
            {settings?.logoUrl ? (
               <img src={settings.logoUrl} alt="Logo" className="w-32 h-32 md:w-40 md:h-40 object-contain drop-shadow-xl" />
            ) : (
            ...
            <h1 className="mt-6 text-2xl md:text-3xl font-black tracking-tight text-foreground opacity-90">
               {settings?.companyName || "PEL ERP"}
            </h1>
            <p className="text-xs uppercase tracking-[0.3em] font-bold text-muted-foreground mt-2 opacity-60">Initializing System</p>
         </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-amber-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-950 dark:to-black">
      ...
      <Card className="w-full max-w-md border-border/50 shadow-2xl backdrop-blur-sm bg-white/80 dark:bg-black/80 rounded-3xl overflow-hidden animate-in fade-in zoom-in duration-500 relative z-10">
        <div className="h-2 bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600"></div>
        
        <CardHeader className="space-y-4 pt-8 text-center relative">
          {settings?.logoUrl ? (
             <img src={settings.logoUrl} alt="Logo" className="mx-auto w-auto h-16 object-contain mb-2" />
          ) : (
          ...
          <div className="space-y-1">
            <CardTitle className="text-3xl font-black tracking-tight text-foreground">
              {settings?.companyName ? settings.companyName.split(' ')[0] : "PEL"}{" "}
              <span className="text-amber-600">{settings?.companyName ? settings.companyName.split(' ').slice(1).join(' ') : "ERP"}</span>
            </CardTitle>
            <CardDescription className="text-sm font-medium text-muted-foreground uppercase tracking-widest line-clamp-1">
              {isRegister ? "Create Admin Account" : (settings?.companyName || "Petroleum Exploration Limited")}
            </CardDescription>
          </div>
        </CardHeader>
