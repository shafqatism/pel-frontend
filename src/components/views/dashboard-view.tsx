...
export default function DashboardView() {
  ...
  const { stats: dashboardStats, isLoading: mainStatsLoading } = useDashboardStats()
  const { data: trends, isLoading: trendsLoading } = useExpenseTrends()
  const { data: hseStats, isLoading: hseLoading } = useHseStats()
  const { data: predictions } = useMaintenancePredictions()

  const stats = [
    { label: "Active Vehicles", value: dashboardStats.vehicles, icon: Truck, change: "+2 this week", trend: "up", color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950/30", loading: mainStatsLoading },
    { label: "Total Manpower", value: dashboardStats.manpower, icon: Users, change: "+4 this month", trend: "up", color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950/30", loading: mainStatsLoading },
    { label: "Company Registry", value: dashboardStats.organizations, icon: Building2, change: "Vendors & Partners", trend: "stable", color: "text-purple-600", bg: "bg-purple-50 dark:bg-purple-950/30", loading: mainStatsLoading },
    { label: "Monthly Expenditure", value: dashboardStats.expenditure ? `₨ ${(dashboardStats.expenditure / 1000000).toFixed(1)}M` : "₨ 0", icon: DollarSign, change: "Current month", trend: "down", color: "text-rose-600", bg: "bg-rose-50 dark:bg-rose-950/30", loading: mainStatsLoading },
  ]
